...
Все три запроса вернут `202 Accepted` **моментально** — Redis принимает их в
очередь и FastAPI сразу отдаёт `task_id`. Celery-воркер сейчас запущен с
`--pool=threads --concurrency=1`, то есть **обрабатывает по одной задаче за
раз**, остальные ждут в очереди. Это сознательный выбор: vLLM на одном GPU
держит одного потребителя стабильнее, чем нескольких параллельных. Чтобы
добавить параллелизм — нужно поднять дополнительные реплики `celery-worker` или
