...
- **Entity Sanitizer**
  ([entity_sanitizer.py](../hse_prom_prog/agents/entity_sanitizer.py)) — слой
...
- **Схема БД сама подтягивается.** Перед каждым запуском агент идёт в
  ...
  [schema_loader.py:22](../hse_prom_prog/agents/schema_loader.py#L22) (сейчас
...
- **До 3 попыток на исправление SQL.** Retry'и могут сработать по двум причинам:
  ...
  - **SQL отработал, но семантически не подходит** под вопрос —
    `_semantic_check` в
    [sql_agent.py:532-544](../hse_prom_prog/agents/sql_agent.py#L532-L544) через
    regex-сигналы ловит mismatch (например, на «у какой команды самый большой
    scope drop?» модель сделала просто `WHERE team_name='cthulhu'` без
