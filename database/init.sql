...
COMMENT ON COLUMN report_agile_dashboard.storypoints_act IS 'текущие story points задачи — используйте SUM для подсчёта SP команды';
...
COMMENT ON COLUMN report_agile_dashboard_metrics.initial_commitment_sp IS 'начальный объём SP спринта (НЕ сумма SP задач)';
COMMENT ON COLUMN report_agile_dashboard_metrics.added_work_sp IS 'добавленные SP';
COMMENT ON COLUMN report_agile_dashboard_metrics.final_commitment_sp IS 'итоговый объём SP спринта (НЕ сумма SP задач)';
COMMENT ON COLUMN report_agile_dashboard_metrics.complete_sp IS 'velocity — выполненные SP за спринт';
