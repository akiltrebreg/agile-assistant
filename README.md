...
пяти LLM-агентов (Supervisor / SQL / RAG / Validator / Response), трёхуровневые
...
- [Memory Layer (кратко)](#memory-layer-кратко)
- [Guardrails (кратко)](#guardrails-кратко)
- [Observability (кратко)](#observability-кратко)
...
- [Конфигурация (кратко)](#конфигурация-кратко)
...
Под капотом — LangGraph-граф из пяти LLM-агентов с детерминированной
маршрутизацией по `query_type`, тремя слоями guardrails (input regex / SQL AST /
output sanitization) и двухуровневой памятью (sliding window + профиль). Сверху
— FastAPI/Celery-стек для асинхронной обработки и Streamlit-чат, наблюдаемые
через Prometheus, Grafana и Langfuse.
...
[guardrails](#guardrails-кратко): regex-фильтр на входе, валидация SQL перед
...
сообщениями память слоя [Memory Layer](#memory-layer-кратко) сохраняет историю
диалога и профиль пользователя — Supervisor и Response Agent читают
короткоживущий контекст + долгоживущий профиль из блока `MemoryManager`. Поверх
этого работает [observability-стек](#observability-кратко): Prometheus + Grafana
собирают метрики latency / throughput / error rate / очередей, а Langfuse
получает трейсы каждого запроса с полными промптами и токенами:
...
dense+sparse) · Celery + Redis (асинхронная очередь) · FastAPI (HTTP API) ·
...
| `rag`       | Ищет ответ в базе знаний через Qdrant        | «Как рассчитывается scope drop?», «Что такое DoD?»    |
| `hybrid`    | Комбинирует данные из БД + контекст из KB    | «Метрики lpop и что можно улучшить»                   |
...
- **Окно диалога** (short-term): sliding window по токен-бюджету
  (`HISTORY_TOKEN_BUDGET=1200`) + rolling summary для всего, что выпало;
...
- **Langfuse** — трейс каждого запроса с полными промптами LLM-вызовов,
  входными/выходными токенами и model parameters. Singleton-клиент с graceful
  degradation (`LANGFUSE_ENABLED=false` → `@observe` становится no-op).
...
Четыре независимых eval-пайплайна — по одному для каждого LLM-агента. Результаты
