...
ветвлением. Supervisor решает, нужен ли запрос к БД или можно ответить напрямую:
...
```
  ...
  │  Supervisor  │ db_query                  └─────────────────┘
  │  (извлекает  │
  │  issue_key,  │ direct_response           ┌─────────────────┐
  │  решает      ├─────────────────────────▶│ Response Agent  │
  │  маршрут)    │                           │  (прямой ответ  │
  └──────────────┘                           │  через LLM)    │
                                             └─────────────────┘
```
...
- Принимает решение о маршруте:
  - `db_query` — если найден ключ задачи, запрос идет в SQL Agent
  - `direct_response` — если ключ не найден, запрос идет сразу в Response Agent
...
- Два режима работы:
  - **С данными из БД**: анализирует данные и генерирует ответ о конкретной
    задаче
  - **Прямой ответ**: отвечает на общие вопросы напрямую через LLM без обращения
    к БД
...
# Вопросы о конкретных задачах (маршрут: Supervisor → SQL Agent → Response Agent)
...
# Общие вопросы (маршрут: Supervisor → Response Agent, без обращения к БД)
...
- Поддержка двух типов запросов:
  - Вопросы о конкретных задачах (например, "Расскажи о задаче AL-38787")
  - Общие вопросы (например, "Что такое спринт?", "Привет")
