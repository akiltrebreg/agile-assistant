...
class SupervisorAgent:
    ...
    def _classify_with_llm(self, user_query: str) -> dict[str, Any]:
        ...
        prompt = (
            ...
            "## Off-topic\n"
            "query_type=off_topic — запрос НЕ связан с Jira, задачами,\n"
            "спринтами, Agile-метриками или практиками разработки.\n"
            "Примеры off_topic: рецепты, погода, стихи, анекдоты, политика,\n"
            "здоровье, финансы, перевод текста, курсы валют.\n"
            "Fail-open: если запрос хоть отдалённо связан с работой команды\n"
            "или процессами разработки — это НЕ off_topic. Сомневаешься —\n"
            "НЕ ставь off_topic (отдай в simple/rag).\n\n"
            "## Алгоритм классификации (следуй строго по шагам)\n"
            "\n"
            "Шаг 0. Запрос явно off-topic (см. блок выше)?\n"
